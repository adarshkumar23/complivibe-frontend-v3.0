...
import { Settings, ShieldCheck, UserPlus, RefreshCw, Download, type LucideIcon } from "lucide-react";
...
export function EnterpriseActions() {
  ...
  return (
    <div className="flex flex-wrap items-center gap-2">
      <NavAction icon={Settings} label="Workspace settings" onClick={() => router.push("/dashboard/settings")} />
      <NavAction icon={ShieldCheck} label="Security" onClick={() => router.push("/dashboard/security")} />
      <DisabledAction icon={UserPlus} label="Invite user" />
      <DisabledAction icon={RefreshCw} label="Rotate key" />
      <DisabledAction icon={Download} label="Export audit logs" />
    </div>
  );
}
