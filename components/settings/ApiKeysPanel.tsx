...
import { normalizeApiKeys } from "@/lib/api/settings-normalizers";
import { formatRelativeTime } from "@/lib/utils/format";
import type { SettingsData } from "@/lib/hooks/useSettings";

export function ApiKeysPanel({ data }: { data: SettingsData }) {
  const { apiKeys } = data;
  const keys = normalizeApiKeys(apiKeys.data);

  return (
    <SectionCard title="API Keys" subtitle="Key metadata (values masked)" icon={KeyRound} accent="cyan" className="h-full">
      {apiKeys.isLoading ? (
        <SkeletonRows rows={4} />
      ) : apiKeys.isError ? (
        <ErrorState title="Unable to load API keys" onRetry={() => apiKeys.refetch()} />
      ) : keys.length === 0 ? (
