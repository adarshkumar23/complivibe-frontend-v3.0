...
export function UploadEvidenceButton({ onUploaded }: { onUploaded?: () => void }) {
  ...
  async function handleFile(file: File | undefined) {
    ...
    try {
      ...
      // 404/405 => endpoint not available; other => real failure. Never fake success.
      if (err instanceof ApiError && (err.status === 404 || err.status === 405 || err.status === 501)) {
        setState("unavailable");
        setMessage("Upload isn't available on this backend yet.");
      } else {
        setState("error");
      ...
      {state !== "idle" && message ? (
        <span
          className={cn(
            ...
            state === "success" ? "text-emerald-600" : state === "error" ? "text-rose-600" : state === "unavailable" ? "text-amber-600" : "text-cv-slate"
          )}
        >
          {state === "success" ? <Check size={12} /> : state === "error" || state === "unavailable" ? <TriangleAlert size={12} /> : null}
          {message}
        </span>
      ) : null}
    </div>
