...
export function PrivacyPoliciesEvidence({ data }: { data: PrivacyData }) {
  ...
  return (
    <SectionCard
      ...
      subtitle="Governance policies & linked evidence"
      icon={ScrollText}
      accent="green"
      className="h-full"
    ...
    >
      {policies.isLoading ? (
        ...
        <EmptyState icon={FileText} title="No policies returned" description="Privacy and governance policies will appear here once available." />
      ) : (
          ...
          {list.slice(0, 12).map((p) => (
            <li key={p.id} className="flex items-start justify-between gap-3 rounded-xl bg-white/55 px-3 py-2.5 ring-1 ring-white/60">
              <div className="min-w-0">
                ...
                <p className="truncate text-[11px] text-cv-slate">{[p.framework || p.category, p.lastUpdated ? `Updated ${formatDate(p.lastUpdated)}` : null].filter(Boolean).join(" · ") || "—"}</p>
              </div>
