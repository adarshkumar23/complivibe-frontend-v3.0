...
export function LinkedRisksIncidents({ data }: { data: AlertsData }) {
  const { risks, incidents } = data;
  const riskItems: Item[] = normalizeRisks(risks.data).filter((r) => !isMitigated(r.status)).map((r) => ({ id: `r-${r.id}`, title: r.title, severity: r.severity, hasSeverity: r.hasSeverity, kind: "Risk" }));
  const incItems: Item[] = normalizeIncidents(incidents.data).filter((i) => !isResolved(i.status)).map((i) => ({ id: `i-${i.id}`, title: i.title, severity: i.severity, hasSeverity: i.hasSeverity, kind: "Incident" }));
  ...
  return (
    <SectionCard title="Linked Risks & Incidents" subtitle="Open items behind the signals" icon={ShieldAlert} accent="red" className="h-full">
      {loading ? (
        <SkeletonRows rows={4} />
      ) : errored ? (
        <ErrorState compact title="Unable to load" onRetry={() => risks.refetch()} />
      ) : combined.length === 0 ? (
        <EmptyState compact icon={ShieldCheck} title="No open risks or incidents" description="Open risks and incidents related to alerts will appear here." />
      ) : (
