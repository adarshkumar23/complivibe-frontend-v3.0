...
export function AlertDeadlines({ data }: { data: AlertsData }) {
  ...
  return (
    <SectionCard title="Regulatory Deadlines" subtitle="Time-sensitive obligations" icon={CalendarClock} accent="amber" className="h-full">
      {deadlines.isLoading ? (
        <SkeletonRows rows={4} />
      ) : deadlines.isError ? (
        <ErrorState compact title="Unable to load deadlines" onRetry={() => deadlines.refetch()} />
      ) : items.length === 0 ? (
        <EmptyState compact icon={CalendarOff} title="No deadlines tracked" description="Upcoming regulatory deadlines will appear here as they are detected." />
      ) : (
