...
import { FileBarChart, PackageCheck, TriangleAlert, FileCheck2, Download, type LucideIcon } from "lucide-react";
...
export function ExecutiveActions() {
  ...
  return (
    <div className="flex flex-wrap items-center gap-2">
      ...
      <NavAction icon={PackageCheck} label="View audit packs" onClick={() => router.push("/dashboard/audit-pack")} />
      ...
      <button type="button" disabled title="Action endpoint unavailable" className="inline-flex cursor-not-allowed items-center gap-1.5 rounded-full bg-white/55 px-3.5 py-2 text-[12px] font-semibold text-cv-mist ring-1 ring-white/60">
        <Download size={13} /> Export executive summary
      </button>
    </div>
  );
}
