...
export default function ApprovalsPage() {
  const { executionApprovals, reviewQueue, envelopes, summary } = useApprovals();
          ...
          {executionApprovals.isLoading ? (
            ...
            <ul className="space-y-2.5">
              {(executionApprovals.data ?? []).map((a, i) => (
                <li key={a.id ?? i} className="flex items-center justify-between gap-3 rounded-xl bg-white/50 px-3 py-2.5 ring-1 ring-white/60">
          ...
          {envelopes.isLoading ? (
            ...
            <ul className="space-y-2.5">
              {(envelopes.data ?? []).map((e, i) => (
                <li key={e.id ?? i} className="flex items-center justify-between gap-3 rounded-xl bg-white/50 px-3 py-2.5 ring-1 ring-white/60">
